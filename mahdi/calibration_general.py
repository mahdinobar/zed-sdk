...
def calc_results(log_dir):
    ...
    P_t_gt_0_measured = np.array([513, -272, 69])
    # TODO
    N_trigger = 10
    bias = np.array(
    ...
    for i in range(N_trigger, 45):
        P_t_gt_all[i, 1] += 0.5  # correcting initial position to move
        for j in range(N_trigger, i):
            P_t_gt_all[i, 1] += (dt[j]) * 34.9028e-3
    for k in range(45, N):
        P_t_gt_all[k, 1] = P_t_gt_all[k - 1, 1]
    P_t_gt_all[45:N, 1] = P_t_gt_all[45:N, 1] + 6.5
